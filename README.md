...
This repository comprises an assortment of modular scripts and configuration files built upon Arch and dwm. Its objective is to deliver a stable, clutter-free, and ergonomic desktop environment tailored for advanced users.
...
4.  (Optional) By default, this script does not check for `.desktop` files in the `xsessions` folder. If you use other display manager like sddm, add a `dwm.desktop` file with below content in the `xsessions` folder:
